...
#
#   alias  The binding alias your project queries at runtime via
#          hub.get_key("<alias>") or the LEAFHUB_ALIAS env var.
#          Defaults to "default" when omitted.
#
#          ⚠ ALIAS IS THE BINDING CONTRACT — mismatch is the most common
#          cause of `credentials: none` after a successful registration:
#
#            leafhub_setup_project "my-app" "$SCRIPT_DIR" "rewrite"
#                                                           ↑
#            hub.get_key("rewrite")  ← runtime code must use the same alias
#
#          If you omit this argument, the binding is created as "default".
#          If your runtime code calls hub.get_key("rewrite") but the binding
#          is "default", you get AliasNotBoundError (shown as: credentials: none).
#
#          Always pass the alias explicitly.  Never rely on the default.
#
#          After leafhub_setup_project completes, verify the binding was
#          created with the correct alias:
#            leafhub project show <name>
#          If missing: leafhub project bind <name> --alias <alias> --provider <name>
#
# ENVIRONMENT
#   LEAFHUB_HEADLESS=1   Skip all interactive prompts.
#                        Set before calling if your setup.sh has --headless.
#                        In headless mode with multiple providers, the first
#                        provider selected is the one with the smallest UUID
#                        (lexicographic order), not the most recently added.
#
# RETURNS
#   0   Registration and binding completed successfully.
#   1   LeafHub install failed, or `leafhub register` returned non-zero.
#       Note: auto-bind failure is non-fatal (project is linked, binding may
#       be missing) — check return value AND verify with `leafhub project show`.
#
# EXAMPLE — default alias (only if runtime code also uses hub.get_key("default"))
#   leafhub_setup_project "my-project" "$SCRIPT_DIR" \
#       || fail "LeafHub registration failed."
#
# EXAMPLE — custom alias (recommended: always explicit)
